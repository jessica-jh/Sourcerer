...
async def find_supporting(
    ...
    *,
    ...
    """Ranks the local library against `sentence`, same two-stage scoring as the
    live pipeline (embedding pre-rank -> claude judge on a shortlist) but with no
    external search, dedupe, or venue filtering -- the library is already the
    full, curated candidate set."""
    if not library:
        return []
